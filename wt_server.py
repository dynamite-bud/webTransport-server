        ...
        if isinstance(event, WebTransportStreamDataReceived):
            ...
            if event.stream_ended:
                ...
                if(self._num_messages==1):
                    print(event.data)
                response_id = event.stream_id
                # payload=str(event.data, 'ascii')[::-1].encode('ascii')
                # print(event.data.hex());
                # print(len(list(event.data)));
                # print(event);
                # payload=event.data[::-1];

                # self._http._quic.send_stream_data(
                #     response_id, payload, end_stream=False)
                # print(event.data);
...
class WebTransportProtocol(QuicConnectionProtocol):
    ...
    def _handshake_webtransport(self,
        ...
        if path == b"/counter":
            ...
            print("Handshaking WebTransport connection")
